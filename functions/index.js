...
app.post('/send', async (req, res) => {
	...
	if (!referral) return res.status(404).send({ message: 'no referral found' });

	//GET CURRENT USER
	if (referral.email_sent)
		return res
			.status(404)
			.send({ message: 'email was already send', success: false });
	...
	//SEND CLOSED SALE REFERRAL
	if (status.name === 'Closed' && !email_sent) {
		return client
			.sendMail({
				to: [manager.email],
			...
			.then((finalResponde) => {
				return res.status(200).send({ message: 'email sent', success: true });
			})
			.catch((err) => console.log('ER', err));
	}

	//SEND REFEREE EMAIL CONGRATULATION;
});

exports.onEmailSent = functions.https.onRequest(app);
